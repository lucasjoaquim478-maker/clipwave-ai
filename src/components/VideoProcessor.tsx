...
import type { ProcessingJob, Clip, ViralMoment } from "@/lib/types";
    ...
    return (
      <div>
        <motion.div
          ...
          className="glass rounded-2xl p-6 border border-green-500/20 mb-8"
        >
          ...
          <p className="text-sm text-white/50">
            {job.clips.length} cortes gerados a partir de &ldquo;{job.videoInfo?.title}&rdquo;
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-8">
          {[
            ...
            >
              <div className={`inline-flex w-10 h-10 rounded-xl bg-gradient-to-br ${stat.color} p-2.5 mb-2`}>
            ...
            <motion.div
              ...
              className={`glass rounded-2xl overflow-hidden border transition-all ${
                selectedClip?.id === clip.id
                  ? "border-primary-500/50 neon-glow"
                  : "border-white/10"
              }`}
            >
              <div className="relative aspect-video bg-gradient-to-br from-gray-900 to-black flex items-center justify-center group cursor-pointer"
                ...
                <div className="absolute top-3 left-3 flex items-center gap-2">
                  ...
                  <span className="glass px-2 py-1 rounded-full text-xs text-white/60">
              ...
              <div className="p-4">
                <div className="flex items-start justify-between mb-2">
                  <h3 className="text-sm font-medium text-white/90 line-clamp-1 flex-1">
                    {clip.title}
                  </h3>
                  <div className="flex gap-1 ml-2">
                    {clip.emojis.slice(0, 3).map((emoji, i) => (
                ...
                <div className="flex items-center gap-2 mb-3">
                  {clip.exportFormats.map((fmt) => (
                    <button
                      key={fmt.platform}
                      onClick={() => handleExport(clip.id, fmt.platform)}
                  ...
                  <button
                    onClick={() => handleExport(clip.id, "mp4")}
                ...
                <button
                  onClick={() => { setSelectedClip(clip); setEditingCaptions(true); }}
        ...
        <div className="flex justify-center mt-8">
          <button
            onClick={() => { setJob(null); setJobId(null); setSelectedClip(null); setUrl(""); }}
    ...
    return (
      <div className="glass rounded-2xl p-8 border border-white/10">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-white/50">
            {job.videoInfo?.title || "Analisando conteúdo..."}
          </p>
        </div>
          ...
          {currentSteps.map((step, i) => (
            <div key={i} className="flex items-center gap-3 text-sm">
              {step.status === "done" ? (
                ...
                <Loader2 className="w-4 h-4 text-neon-blue animate-spin flex-shrink-0" />
              ) : (
                <div className="w-4 h-4 rounded-full border border-white/10 flex-shrink-0" />
              )}
              <span className={`${step.status === "done" ? "text-white/60" : step.status === "active" ? "text-white/80" : "text-white/30"}`}>
                {step.label}
              </span>
            </div>
          ))}
  ...
  if (job?.status === "error") {
    return (
      <div className="glass rounded-2xl p-8 border border-red-500/20 text-center">
        ...
        <button onClick={() => { setJob(null); setJobId(null); }} className="btn-primary">
          Tentar Novamente
        </button>
      </div>
    );
  ...
  return (
    <div className="glass rounded-2xl p-6 sm:p-8 border border-white/10">
      <div className="max-w-2xl mx-auto text-center mb-8">
        ...
        <p className="text-sm text-white/50">
          A IA vai baixar, analisar e criar cortes virais automáticos
        </p>
      </div>
      ...
      <div className="max-w-xl mx-auto">
        ...
        <div className="mt-6 flex flex-wrap items-center justify-center gap-3 text-xs text-white/30">
          <span className="flex items-center gap-1">
            <Clock className="w-3 h-3" /> Até 4K
          </span>
          <span className="flex items-center gap-1">
            <Film className="w-3 h-3" /> Lives de até 3h
          </span>
          <span className="flex items-center gap-1">
            <Sparkles className="w-3 h-3" /> Cortes automáticos
          </span>
        </div>
      </div>
    </div>
